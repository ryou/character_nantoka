...
const paths = {
  sprite: {
    src: 'src/sprite/to_sprite',
    dest: './src/static/assets/sprite',
    tmp: 'tmp/sprite',
  },
};
...
module.exports = () => {
  ...
  gulp.task('sprite.sprite', ['sprite.clean'], (cb) => {
    ...
    const copyOriginal = (folder) => {
      return new Promise((resolve) => {
        let tmp = gulp.src(paths.sprite.src + '/' + folder + '/**/*.png');
        if (folder === 'eye' || folder === 'eye_color' || folder === 'face') {
          tmp = tmp.pipe(gm(function(gmfile) {
            return gmfile.crop(300, 200, 0, 0);
          }, {
            imageMagick: true,
          }))
        }
        tmp.pipe(gulp.dest(paths.sprite.tmp + '/original/' + folder))
          .on('end', resolve);
      });
    };
    const copy = (folder) => {
      return new Promise((resolve) => {
        gulp.src(paths.sprite.tmp + '/original/' + folder + '/**/*.png')
          .pipe(rename({
            suffix: '@2x',
          }))
